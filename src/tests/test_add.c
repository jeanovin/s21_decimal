...
START_TEST(test_add_decimal_combined) {
  ...
  } cases[] = {{{0b100001000, 0b1110011100, 0b10001},
                {0b1011111001111, 0b1111000011100011, 0b1111111111},
                "19184614105484231710935"},
               {{0b10111110001000, 0b1110011100, 0b100111101},
                {0b1011110001001111, 0b111001000011100011, 0b1111100011111},
                "152812828914308037536727"},
               {{0b1000011111000, 0b11100, 0b101},
                {0b101001111, 0b1111100011, 0b111111001},
                "9407839481985622872647"}};

  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
    s21_decimal decimal1 = {
        {cases[i].decimal1[0], cases[i].decimal1[1], cases[i].decimal1[2], 0}};
    s21_decimal decimal2 = {
        {cases[i].decimal2[0], cases[i].decimal2[1], cases[i].decimal2[2], 0}};
    s21_decimal result;
    s21_add(decimal1, decimal2, &result);
...
START_TEST(test_add_decimal_overbig_combined) {
  struct {
    ...
    const char *expected_str;
      ...
      {{0b11111111111111111111111100001000, 0b11111111111111111111111100001000,
      ...
      {{0b11111111111111111111111100001000, 0b11111111111111111111111100001000,
        0b11111111111111111111111100001000},
       {0b101111111111111001111, 0b111100001111111111100011,
        0b1111111111111111111},
       1,
       NULL},
      {{0b11111111111111111111111111111111, 0b11111111111111111111111100001000,
        0b11111111111111111111111100001000},
       {0b101111111111111001111, 0b111100001111111111100011,
        0b1111111111111111111},
       1,
  ...
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
    s21_decimal decimal1 = {{cases[i].decimal1[0], cases[i].decimal1[1],
                             cases[i].decimal1[2], (i == 4 ? 0x80000000 : 0)}};
    s21_decimal decimal2 = {
        {cases[i].decimal2[0], cases[i].decimal2[1], cases[i].decimal2[2], 0}};
    s21_decimal result;
...
START_TEST(test_add_decimal_overbig_combined_neg) {
  ...
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
    s21_decimal decimal1 = {{cases[i].decimal1[0], cases[i].decimal1[1],
                             cases[i].decimal1[2], cases[i].sign}};
    s21_decimal decimal2 = {{cases[i].decimal2[0], cases[i].decimal2[1],
                             cases[i].decimal2[2], cases[i].sign}};
    s21_decimal result;
    int code = s21_add(decimal1, decimal2, &result);
    ck_assert_int_eq(code, cases[i].expected_code);
  }
...
START_TEST(test_add_decimal) {
  ...
  AddDecimalTestParams test_cases[] = {
      ...
      {0b0, 0b0, 0b11111111111111111111111111111111, 0b0, 0b0, 0b0,
       "79228162495817593519834398720"},
      {0b11111111111111111111111111111111, 0b11111111111111111111111111111111,
       0b11111111111111111111111111111111, 0b0, 0b0, 0b0,
       "79228162514264337593543950335"},
      {0b0, 0b0, 0b0, 0b11111111111111111111111111111111,
       0b11111111111111111111111111111111, 0b11111111111111111111111111111111,
       "79228162514264337593543950335"},
  ...
  for (size_t i = 0; i < sizeof(test_cases) / sizeof(test_cases[0]); i++) {
    ...
    s21_decimal decimal1 = {
        {test_case.decimal1_1, test_case.decimal1_2, test_case.decimal1_3, 0}};
    s21_decimal decimal2 = {
        {test_case.decimal2_1, test_case.decimal2_2, test_case.decimal2_3, 0}};
    s21_decimal result;
    s21_add(decimal1, decimal2, &result);
...
START_TEST(test_add_decimal_oversmall) {
  ...
  AddDecimalTestParams test_cases[] = {
      {0b1, 0b0, 0b0, 0b10000000000111000000000000000000, 0b0, 0b0, 0b0, 0b0,
       "-0.0000000000000000000000000001", 0},
      {0b1, 0b0, 0b0, 0b111000000000000000000, 0b0, 0b0, 0b0, 0b0,
       "0.0000000000000000000000000001", 0},
      {0b1, 0b0, 0b0, 0b10000000000111000000000000000000, 0b1, 0b0, 0b0,
       0b00000000000111000000000000000000, "0", 0},
      {0b1, 0b0, 0b0, 0b10000000000111000000000000000000, 0b1, 0b0, 0b0,
       0b10000000000111000000000000000000, "-0.0000000000000000000000000002",
       0},
      {0b111111, 0b0, 0b0, 0b00000000000111000000000000000000, 0b111, 0b0, 0b0,
       0b10000000000111000000000000000000, "0.0000000000000000000000000056", 0},
      {0b111111, 0b0, 0b0, 0b00000000001111000000000000000000, 0b111, 0b0, 0b0,
       0b10000000000111000000000000000000, "", 4}};

  for (size_t i = 0; i < sizeof(test_cases) / sizeof(test_cases[0]); i++) {
    AddDecimalTestParams test_case = test_cases[i];
    s21_decimal decimal1 = {{test_case.decimal1_1, test_case.decimal1_2,
                             test_case.decimal1_3, test_case.decimal1_4}};
    s21_decimal decimal2 = {{test_case.decimal2_1, test_case.decimal2_2,
                             test_case.decimal2_3, test_case.decimal2_4}};
    s21_decimal result;
    int code = s21_add(decimal1, decimal2, &result);
...
START_TEST(test_add_decimal_smallpow) {
  ...
  AddDecimalTestParams test_cases[] = {
      {0b11111111111111111111111111111111, 0b11111111111111111111111111111111,
       ...
       0b1, 0b0, 0b0, 0b10000000000111000000000000000000,
       "7.9228162514264337593543950334"},
      {0b11111111111111111111111111111111, 0b11111111111111111111111111111111,
       0b11111111111111111111111111111111, 0b00000000000111000000000000000000,
       0b1, 0b0, 0b0, 0b00000000000111000000000000000000,
      ...
      {0b11111111111111111111111111111111, 0b11111111111111111111111111111111,
       ...
       0b00000000000111000000000000000000, "7.922816251426433759783891763"},
      {0b11111111111111111111111111111111, 0b11111111111111111111111111111111,
       0b11111111111111111111111111111111, 0b00000000000111000000000000000000,
       0b11111111111111111111111111111111, 0b11111111111111111111111111111111,
       0b11111111111111111111111111111111, 0b00000000000111000000000000000000,
